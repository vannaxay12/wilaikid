...
router.get('/dashboard', auth, adminOnly, async(_,res)=>{
  try {
    ...
    const [[todaySales]] = await db.query(`SELECT COALESCE(SUM(total_amount),0) AS revenue,COUNT(*) AS bills FROM sales WHERE DATE(sale_datetime)=?`,[today]);
    const [[monthSales]] = await db.query(`SELECT COALESCE(SUM(total_amount),0) AS revenue FROM sales WHERE MONTH(sale_datetime)=MONTH(NOW()) AND YEAR(sale_datetime)=YEAR(NOW())`);
    const [[lowStock]]   = await db.query(`SELECT COUNT(*) AS cnt FROM products WHERE stock_qty<=min_stock_level AND is_active=1`);
    const [[totalProd]]  = await db.query(`SELECT COUNT(*) AS cnt FROM products WHERE is_active=1`);
    const [topProducts]  = await db.query(`SELECT p.product_name,SUM(si.qty) AS qty_sold,SUM(si.qty*si.unit_price) AS revenue
      FROM sale_items si JOIN products p ON si.product_id=p.product_id GROUP BY si.product_id ORDER BY revenue DESC LIMIT 5`);
    const [dailyChart]   = await db.query(`SELECT DATE(sale_datetime) AS day,SUM(total_amount) AS revenue
      FROM sales WHERE sale_datetime>=DATE_SUB(NOW(),INTERVAL 7 DAY) GROUP BY day ORDER BY day`);
...
router.get('/stock', auth, adminOnly, async(_,res)=>{
  try {
    const [rows] = await db.query(`
      ...
      WHERE p.is_active=1
      ORDER BY FIELD(stock_status,'ໝົດ','ຕ່ຳ','ກາງ','ສູງ'), p.product_name`);
    res.json(rows);
  } catch(e){ res.status(500).json({message:e.message}); }
});
...
router.get('/purchases', auth, adminOnly, async(req,res)=>{
  try {
    ...
    let sql = `SELECT po.*, CONCAT(e.first_name,' ',e.last_name) AS employee_name,
      ...
      WHERE 1=1`;
    const p = [];
    if (from) { sql+=' AND po.order_date>=?'; p.push(from); }
    if (to)   { sql+=' AND po.order_date<=?'; p.push(to); }
...
router.get('/inventory', auth, adminOnly, async(req,res)=>{
  try {
    ...
    let sql = `SELECT ir.*, CONCAT(e.first_name,' ',e.last_name) AS employee_name,
      ...
      WHERE 1=1`;
    const p = [];
    if (from) { sql+=' AND DATE(ir.received_at)>=?'; p.push(from); }
    if (to)   { sql+=' AND DATE(ir.received_at)<=?'; p.push(to); }
