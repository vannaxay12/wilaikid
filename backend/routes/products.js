...
// GET /api/products
//   ແບບໃໝ່: ເອົາ authAny ອອກ ເພື່ອໃຫ້ຄົນຍັງບໍ່ Login ເຂົ້າເບິ່ງສິນຄ້າໄດ້
...
router.post('/', auth, adminOnly, async(req, res) => {
    try {
       ...
       VALUES (?,?,?,?,?,?,?,?,?)`, [barcode || null, product_name, category_id, unit_id, cost_price, selling_price, stock_qty || 0, min_stock_level || 5, expiry_date || null]
        );
...
router.put('/:id', auth, adminOnly, async(req, res) => {
    try {
       ...
       cost_price=?,selling_price=?,min_stock_level=?,expiry_date=? WHERE product_id=?`, [barcode || null, product_name, category_id, unit_id, cost_price, selling_price, min_stock_level || 5, expiry_date || null, req.params.id]
        );
        res.json({ ok: true });
    } catch (e) { res.status(500).json({ message: e.message }); }
});
